...
export default function DashboardPage() {
  ...
  const fetchListings = async (uid: string) => {
    ...
    const { data } = await supabase
      .from('listings').select('*').eq('seller_id', uid)
  ...
  return (
    <div style={{ background: '#FAF7F2', minHeight: '100vh' }}>
      ...
      <section className="bg-[#FAF7F2] border-b border-[#EFE9DD]">
        <div className="max-w-[1180px] mx-auto px-6 pt-12 pb-8">
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <h1 className="m-0 mb-1 text-3xl font-bold tracking-tight text-[#1B4332]">My listings</h1>
          ...
          <div className="flex gap-4 mt-8 flex-wrap">
            {[
              ...
              <div key={s.label} className="bg-white border border-[#EFE9DD] rounded-xl px-5 py-4 min-w-[140px]">
      ...
      <div className="max-w-[1180px] mx-auto px-6 py-10">
        ...
        {loading && (
          <p className="text-center text-[#6B6B6B] py-16">Loading your listings...</p>
        )}
        ...
        {!loading && listings.length === 0 && (
          <div className="bg-white border border-[#EFE9DD] rounded-2xl p-16 text-center">
            ...
            <p className="m-0 mb-6 text-[#6B6B6B]">Post your first item — it goes live immediately.</p>
            <Link href="/sell"
        ...
        {!loading && activeListings.length > 0 && (
          <div className="mb-10">
            <h2 className="m-0 mb-4 text-lg font-bold text-[#1B4332]">Active — visible to buyers</h2>
            <div className="flex flex-col gap-3">
              {activeListings.map(l => (
                <div key={l.id} className="bg-white border border-[#EFE9DD] rounded-2xl p-5 flex gap-4 items-start flex-wrap">
                  {l.image_url ? (
                    <img src={l.image_url} alt={l.title} className="w-20 h-20 object-cover rounded-xl flex-shrink-0" />
                  ) : (
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <p className="m-0 text-lg font-bold text-[#D97706] mt-0.5">{l.price.toLocaleString()} XAF</p>
                    <div className="flex gap-2 mt-2 flex-wrap">
                      <ConditionBadge value={l.condition} />
                      <span className="text-xs bg-[#F1EFE8] text-[#5F5E5A] px-2.5 py-1 rounded-full font-medium">{l.category}</span>
                    </div>
                  </div>
                  <div className="flex gap-2 flex-shrink-0 flex-wrap">
                    ...
                    <button onClick={() => markSold(l.id)} disabled={actionLoading === l.id + '-sold'}
                    ...
                    <button onClick={() => deleteListing(l.id)} disabled={actionLoading === l.id + '-delete'}
        ...
        {!loading && soldListings.length > 0 && (
          <div>
            <h2 className="m-0 mb-4 text-lg font-bold text-[#6B6B6B]">Sold / inactive — hidden from buyers</h2>
            <div className="flex flex-col gap-3">
              {soldListings.map(l => (
                <div key={l.id} className="bg-white border border-[#EFE9DD] rounded-2xl p-5 flex gap-4 items-start flex-wrap opacity-60">
                  {l.image_url ? (
                    <img src={l.image_url} alt={l.title} className="w-20 h-20 object-cover rounded-xl flex-shrink-0" />
                  ) : (
                    <div className="w-20 h-20 bg-[#E2D9C5] rounded-xl flex-shrink-0 flex items-center justify-center text-[10px] font-semibold uppercase tracking-widest text-[#888]">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-0.5">
                      <p className="m-0 font-bold text-[#1A1A1A] truncate">{l.title}</p>
                      <span className="text-xs bg-[#E6F3EA] text-[#1B4332] px-2.5 py-1 rounded-full font-semibold flex-shrink-0">Sold</span>
                    </div>
                    <p className="m-0 text-lg font-bold text-[#D97706] mt-0.5">{l.price.toLocaleString()} XAF</p>
                    <div className="flex gap-2 mt-2 flex-wrap">
                      <ConditionBadge value={l.condition} />
                  ...
                  <div className="flex gap-2 flex-shrink-0 flex-wrap">
                    <button onClick={() => markActive(l.id)} disabled={actionLoading === l.id + '-active'}
